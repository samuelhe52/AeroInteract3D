...
- 实现抓取、移动、旋转和双手缩放等基础交互能力，并提供明确视觉反馈。
- 模块化设计，使系统具备可测试、可维护、可扩展的工程特征。

## 模块概述

- **Gesture 模块**（`src/gesture/`）：系统输入端。从摄像头采集视频帧，调用 MediaPipe Hand Landmarker 检测手部关键点，稳定坐标与交互状态，生成标准化 `GesturePacket`，供下游消费。
- **Bridge 模块**（`src/bridge/`）：语义转换层，系统的"胶水"。消费 `GesturePacket`，维护可交互对象状态表，进行坐标映射，计算用于 3D 渲染的坐标，并生成 `SceneCommand` 序列，供渲染模块消费。
...
`blur_level` 贯穿后续多个环节，以此实现自适应的稳定性调整。由于坐标稳定算法对系统端到端延迟有一定影响，系统需要在保证稳定性的同时尽量降低响应时间。`blur_level` 的引入使得系统能够在模糊帧时自动增强坐标平滑程度（通过动态调整参数实现），而在清晰帧时保持较低的平滑强度以保证响应速度。
...
旋转模式由“握拳后张开手掌”这一特定手势序列触发：先保持手部握拳（指尖聚拢）数帧，再张开手掌，即可切换旋转模式。在该模式下，捏合点的拖拽位移不再驱动对象位置移动，而是经由 Bridge 层转化为物体姿态参数，用于更新物体旋转姿态。

注：对于缩放手势的实现，本项目没有在 Gesture 层直接输出场景缩放命令。Gesture 模块负责检测主手和副手，并将副手状态写入 `GesturePacket.debug` 中的 `secondary_hand` / `dual_hand` 结构；Bridge 层再根据两只手的 pinch 状态、锚点距离和对象捕获关系，将双手间距变化转化为缩放命令。
...
- 画面中有两只手时，可以实现缩放物体效果：当两只手满足 pinch 条件，且当前存在已交互对象或至少一只手悬停在可交互对象上时，双手间距变化会被转换为对物体的缩放命令。
...
在拖拽对象时还会结合桌面高度和平面约束，避免对象穿透桌面：使用 `_constrain_object_to_table` 把对象中心的 y 坐标限定在桌面以上，使交互结果符合桌面场景的基本空间约束。
...
当主手和副手同时满足 pinch 条件时，Bridge 层会优先进入双手缩放路径：若存在上一轮交互对象，则绑定该对象；否则使用当前主手或副手悬停的对象。随后 Bridge 基于两只手的捏合锚点间距变化计算缩放比例，并通过 `set_object_pose` 命令更新对象的 `scale` 字段，实现跟手的缩放效果。缩放操作期间，对象平移与旋转被暂停，以避免命令冲突。
...
Rendering 模块实现了一套多视图 UI 系统，主要交互由指尖锚点驱动；同时，标定页保留键盘快捷键用于精调参数：
...
渲染模块支持导入外部 3D 模型文件（`.glb`、`.egg`、`.bam` 等格式），作为可交互场景对象使用。用户可以将自定义模型放置于 `assets/custom_models/` 目录，系统初始化时会自动扫描并注册，与内置几何体一样参与场景管理和手势交互。当前 `assets/custom_models/` 目录中已包含 `apple`、`lemon`、`croissant`、`potted_plant`、`wooden_table` 等多个自定义模型。
...
可以看到，主循环中不断调用 `render_output.step()` 来驱动渲染更新；通过 `gesture_input.poll()` 获取最新的手势数据包；将手势数据和相机预览帧同步给渲染模块；再把数据包传给 Bridge 处理成场景命令，并将命令逐条推送给渲染模块。

项目运行时，支持通过 YAML 配置文件设置默认参数（摄像头编号、分辨率、画面是否镜像、灵敏度等）。

# 性能评价

在测试环境中，本系统在 1080p 分辨率下通常可达到约 **25 FPS**；关闭渲染模块的调试视图后，帧率可接近 **30 FPS**，用户体验较为流畅。对于手部检测稳定性，在光线较为充足的环境下，系统较少出现明显的跟踪丢失或跳变；在光线较弱或手部快速移动时，如果叠加清晰度/帧率较低的摄像头输入，检测稳定性会有所下降，但由于引入了 fallback 预测机制，整体上能够以延迟小幅增加、灵敏度小幅下降的代价保持交互的连续性和可用性。
...
本项目完成了 MediaPipe 手势检测、坐标稳定、深度估算与 Panda3D 场景渲染的完整系统链路。系统以共享契约划定模块边界，Gesture、Bridge、Rendering 三层可独立开发和测试。Gesture 层通过模糊感知平滑、fallback 预测和概率式 pinch 状态机，将不稳定的视觉观测转化为可靠的交互信号；Bridge 层处理坐标映射、对象状态管理和旋转/缩放语义转换；Rendering 层实现了抓取、平移、旋转、缩放的三维交互场景，以及多视图手势驱动 UI 和自定义模型导入能力。
