...
from panda3d.core import Loader, Material, NodePath
...
class ModelResourceFactory:
    ...
    def auto_scan_and_register(self, models_dir: str) -> None:
        ...
        for filename in sorted(os.listdir(models_dir)):
            ...
            if not os.path.isfile(file_path):
                continue
            if not filename.lower().endswith(self._SUPPORTED_CUSTOM_MODEL_FORMATS):
                continue
        ...
        for shape_id, file_paths in scanned_files_by_shape.items():
            ...
            if shape_id in self._template_registry:
                ...
                continue
            template = self._build_template_from_scanned_model(file_paths[0])
    ...
    @staticmethod
    def _parse_optional_vec3(value: Any, *, field_name: str, model_file_path: str) -> Optional[tuple[float, float, float]]:
    ...
    @staticmethod
    def _parse_optional_positive_vec3(value: Any, *, field_name: str, model_file_path: str) -> Optional[tuple[float, float, float]]:
        parsed = ModelResourceFactory._parse_optional_vec3(value, field_name=field_name, model_file_path=model_file_path)
        if parsed is None:
            return None
    ...
    @staticmethod
    def _parse_optional_bool(value: Any, *, field_name: str, model_file_path: str) -> Optional[bool]:
    ...
    @staticmethod
    def _parse_optional_str(value: Any, *, field_name: str, model_file_path: str) -> Optional[str]:
    ...
    def _load_sidecar_config(self, model_file_path: str) -> dict[str, Any]:
        ...
        if "display_name" in payload:
            parsed_value = self._parse_optional_str(payload["display_name"], field_name="display_name", model_file_path=model_file_path)
            if parsed_value is not None:
                config["display_name"] = parsed_value
        if "default_scale" in payload:
            parsed_value = self._parse_optional_positive_vec3(payload["default_scale"], field_name="default_scale", model_file_path=model_file_path)
            if parsed_value is not None:
                config["default_scale"] = parsed_value
        if "center_offset" in payload:
            parsed_value = self._parse_optional_vec3(payload["center_offset"], field_name="center_offset", model_file_path=model_file_path)
            if parsed_value is not None:
                config["center_offset"] = parsed_value
        if "two_sided" in payload:
            parsed_value = self._parse_optional_bool(payload["two_sided"], field_name="two_sided", model_file_path=model_file_path)
            if parsed_value is not None:
                config["two_sided"] = parsed_value
        if "use_builtin_materials" in payload:
            parsed_value = self._parse_optional_bool(payload["use_builtin_materials"], field_name="use_builtin_materials", model_file_path=model_file_path)
            if parsed_value is not None:
